...
func (c *cartRepository) ReadCartItemsById(ctx context.Context, tx *sql.Tx, cartID uint) ([]entity.CartItem, error) {
	...
	var cartItems = make([]entity.CartItem, 0)
	for row.Next() {
...
func (c *cartRepository) UpdateCartItemQuantity(ctx context.Context, tx *sql.Tx, cartID uint, productID uint, quantity int) error {
	if quantity <= 0 {
...
func (c *cartRepository) RemoveCartItem(ctx context.Context, tx *sql.Tx, cartID uint, productID uint) error {
	now := time.Now().Format(constant.TimeFormat)
...
func (c *cartRepository) ValidateCartItems(ctx context.Context, tx *sql.Tx, cartID uint) error {
